...
sampleGeneExpression2 = function(group1_size = 15, group2_size = 15, genes = 300, noise = 1, average_difference = 1, noise_difference = 1, module1_size = 40, module2_size = 45, batch_size = NULL, batch_average_difference = 1, group1_in_batch = 0.5, seed = NULL, to_return = F){
  ...
  return(list(input = match.call(), gene_expression = X, samples = samples, gene_modules = gene_modules, true_effect = true_effect, batch = if(!is.null(batch_size)) batch else NULL, batch_effect = if(!is.null(batch_size)) batch_effect else NULL))
}
